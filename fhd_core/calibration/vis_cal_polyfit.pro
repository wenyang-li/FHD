FUNCTION vis_cal_polyfit,cal,obs,cal_step_fit=cal_step_fit,cal_neighbor_freq_flag=cal_neighbor_freq_flag,$
    ...
    cal_reflection_mode_file=cal_reflection_mode_file,cal_reflection_mode_delay=cal_reflection_mode_delay,auto_divided=auto_divided,$
  ...
  IF Keyword_Set(cal_mode_fit) THEN BEGIN
    ...
    FOR pol_i=0,n_pol-1 DO BEGIN
      ...
      IF Keyword_Set(auto_divided) THEN BEGIN
        print, 'auto_divided set, only fit the reflection mode in the phase'
      ...
      FOR ti=0L,nt_use-1 DO BEGIN
        ...
        IF Keyword_Set(auto_divided) AND Keyword_Set(cal_reflection_hyperresolve) THEN BEGIN

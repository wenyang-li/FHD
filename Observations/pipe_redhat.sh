...
message=$(sbatch --account=jpober-condo --mem=$mem -t ${wallclock_time} -n ${ncores} -C intel -N 1 --array=0-$(($nobs - 1))%10 --export=ncores=$ncores,outdir=$outdir,version=$version,thresh=$thresh -o ${outdir}/fhd_${version}/grid_out/firstpass-%A_%a.out -e ${outdir}/fhd_${version}/grid_out/firstpass-%A_%a.err ${FHDpath}Observations/eor_firstpass_redhat_job.sh ${good_obs_list[@]})

...
message=$(sbatch -p default-batch --mem=$mem -t ${wallclock_time} -n ${ncores} -C intel --exclude=node934 -N 1 --array=0-$(( $nobs - 1 ))%12 --export=ncores=$ncores,outdir=$outdir,version=$version,thresh=$thresh -o ${outdir}/fhd_${version}/grid_out/array_sim-%A_%a.out -e ${outdir}/fhd_${version}/grid_out/array_sim-%A_%a.err ${FHDpath}simulation/simulation_wrappers/eor_simulation_slurm_job.sh ${good_obs_list[@]})
...
if [ "$rerun_flag" -ne 1 ];then 
   ...
   message=$(sbatch -p default-batch --mem=$mem -t ${wallclock_time} -n ${ncores} -C intel -N 1 --array=0-$(( $nobs - 1 ))%6 --export=ncores=$ncores,outdir=$outdir,version=$version,thresh=$thresh -o ${outdir}/fhd_${version}/grid_out/array_sim-%A_%a.out -e ${outdir}/fhd_${version}/grid_out/array_sim-%A_%a.err ${FHDpath}Observations/eor_simulation_slurm_job.sh ${resubmit_list[@]})
   message=($message)
   id=`echo ${message[3]}`

...
time_array = np.linspace(2456528.21023,2456528.29565,5)
poi_array = ['-2','-1','0','1','2']
#
...
datafile_name='/users/wl42/IDL/FHD/instrument_config/obsolete_txt/mwa_cable_length.txt'
...
for poi_i, poi in enumerate(poi_array):
	...
	#
	datafile_name='/users/wl42/IDL/FHD/instrument_config/obsolete_txt/'+poi+'_bandpass_2013longrun_Jan2017.txt'
	datafile = open(datafile_name, "r")
	cols = (col.strip().split() for col in datafile)
	rows = zip(*cols)
...
#
cal.history = 'EoR0 highband per frequency, per pointing, per polarization bandpass for MWA PhaseII, averaged per cable over 2016. Pointing JD is referenced from Aug 23,2013.'
#
cal.Nspws = 1
...
cal.flag_array = flags.reshape((Nants_data,1,Nfreqs,Ntimes,Njones))
cal.gain_array = gains.reshape((Nants_data,1,Nfreqs,Ntimes,Njones))
cal.quality_array = chisq.reshape((Nants_data,1,Nfreqs,Ntimes,Njones))
#
# Put your name in as creator
#
cal.observer = 'WL'
cal.cal_style = 'sky'
cal.ref_antenna_name = '11'
cal.sky_catalog = 'master_sgal_cat'
cal.sky_field = 'EoR0'
#
# Put in the git url of the code that generated the cals
#
cal.git_origin_cal = 'https://github.com/wenyang-li/FHD'
cal.spw_array = np.ones((1,),dtype=int)
#
...
cal.write_calfits('/users/wl42/IDL/FHD/instrument_config/MWAII_EoR0_highband_2016_cable_bandpass.fits')
